<script>
   ...
   */
    ...
    async function CargarPersonajes(){
        loading = true
        const res = await fetch('https://apisimpsons.fly.dev/api/personajes?limit=20&page='+page)
    ...
    async function Buscar(){
        if (name.length != 0){
            loading = true
            const res = await fetch('https://apisimpsons.fly.dev/api/personajes/find/'+name)
            const data = await res.json()
            console.log(data)
